...
5. **Reflection resistance via routing** — a malicious relay cannot reflect a dialer's ICE candidate back to the dialer by re-posting it with `from: dialerPubkey`. The dialer has no PeerConnection keyed under its own pubkey, so the message is dropped as an unrecognized offer. If the relay changes `from` to the answerer's pubkey, the signature verification fails — the candidate was signed with the dialer's private key. The session challenge already uniquely binds candidates to one negotiation, so no direction byte is needed.

6. **Timestamp freshness** — both offers and answers carry `ts`, an 8-byte big-endian uint64 unix timestamp. The receiver rejects messages with `ts` outside a ±30s window. Offers require freshness to prevent delayed replay of a valid dial. Answers require freshness to maintain symmetric freshness guarantees across the exchange. ICE candidates do not carry `ts` — freshness is guaranteed by the session challenge, which is unique per dial.

7. **Authorization callback** — after signature and timestamp verification, the answerer calls `AcceptConnection(dialer_pubkey)` before creating a PeerConnection or sending an answer. This gives the application control over who can initiate connections. Returning false drops the offer silently (no error response to the dialer) to avoid leaking information about whether the key is active.

8. **Auth failure closes the PC** — any verification failure (invalid signature, wrong or missing challenge, stale timestamp, unacceptable pubkey) causes the library to call `pc.close()` immediately and discard the message. The application may observe the connection close if auth fails on a message received after `onIncoming` fires. `onIncoming` means "a valid authenticated offer arrived and an answer was sent" — not "a working P2P connection exists." The connection is established only after DTLS completes.

9. **Wire message format** — the `ts` field is present in offers and answers, absent from ICE candidates. The `challenge` field is present in all three. The `from` field is self-reported; authenticity is established entirely by the signature verifying against the claimed pubkey. Receivers verify the signature against the raw `data` field bytes as received — no re-serialization before verification.
